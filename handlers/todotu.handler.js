...
import TodoTuModel from '../models/todotu.model.js';

const handler = {};
handler.getList = async (req, res) => {
    const todoList = await TodoTuModel.find();
    return res.json(todoList);
};

handler.postTodo = async (req, res) => {
    // Lấy dữ liệu từ req.body
    const newTodo = await TodoTuModel.create(req.body);
...
handler.patchTodo = async (req, res) => {
    const updatedTodo = await TodoTuModel.findOneAndUpdate(
        { _id: req.params.id },
        { ...req.body },
    );
...
handler.deleteTodo = async (req, res) => {
    const resp = await TodoTuModel.deleteOne({ _id: req.params.id });
